...
abstract class BaseType {
	...
	public function __get($name) {
		...
		if (!method_exists($this, $method)) {
			throw new Exception\InternalSecurityException('No existe el método ' . $method . ' en la clase "' . get_class($this) . '"');
		}
		return $this->$method();
	}
	...
	private function validateConstraints($val) {
		if ( is_array($this->constraints) ) {
			foreach ( $this->constraints as $constraintName => $constraintValue ) {
				$constraint = __NAMESPACE__ . '\Constraint\\' . $constraintName;
	...
	public static function isTyped($value){
		return is_a($value, __NAMESPACE__ . '\BaseType');
	}
